...
const Button: FC<ButtonProps> = memo(({ className, children, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full md:w-auto md:mx-auto flex justify-center items-center p-4 space-x-2 font-sans font-bold text-white rounded-md px-9 bg-orange-600  shadow-cyan-100 hover:bg-opacity-90 shadow-sm hover:shadow-lg border"
    >
      {children}
    </button>
  );
