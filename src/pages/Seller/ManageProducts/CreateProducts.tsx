...
const CreateProducts: FC<YourComponentProps> = memo(() => {
  ...
  const [selectedChooseOptions1, setSelectedChooseOptions1] = useState<
    string[]
  >([]);
  const [selectedChooseOptions2, setSelectedChooseOptions2] = useState<
    string[]
  >([]);
  ...
  const { data: chooseOptions1, isFetching: isLoading1 } =
    useChooseOptions1(selectedOption1);

  const { data: chooseOptions2, isFetching: isLoading2 } =
    useChooseOptions2(selectedOption2);
  ...
  const isVideoValid = async (file: File): Promise<boolean> => {
    ...
    if (file.size > maxSizeMB * 1024 * 1024) {
      setErrorVideo(
        `Kích thước tệp vượt quá ${maxSizeMB} MB. Vui lòng chọn một tập tin nhỏ hơn.`
      );
      return false;
    }
    ...
    if (video.duration > maxDurationSec) {
      setErrorVideo(
        `Độ dài video vượt quá ${maxDurationSec} giây. Vui lòng chọn video ngắn hơn.`
      );
      return false;
    }
      ...
      <div className="p-4 sm:ml-64 mt-16 relative">
        ...
        {mutation.isSuccess && (
          <SuccessAdd
            ...
            onDismiss={handleMutationSuccess}
          ></SuccessAdd>
        )}
        {mutation.isError && (
          <FailureAdd
        ...
        <form className="max-w-3xl mx-auto" onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <div>
              ...
              {errorImg != "" && (
                <p className="text-sm text-red-500">{errorImg}</p>
              )}
            </div>
          </div>
          <div className="mb-4">
          ...
          <div className="flex mb-4 justify-between">
            ...
            <div>
              ...
              <select
                ...
                name="type"
                onChange={handleChange}
                className="flex items-center  justify-between shadow-sm bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 w-48 p-2.5"
              >
                <option selected value="item">
                  Sản phẩm
                </option>
                <option value="box">Hộp quà</option>
                <option value="card">Thiệp lời chúc</option>
              </select>
              {/* <DropdownCategory
          ...
          <div className="flex mb-4 space-x-10">
            ...
            {isLoading1 ? (
              ...
              <div
                id="choose-option1"
                className={`space-y-5 mt-7 ${isVisibleOption1 ? "" : "hidden"}`}
              >
                <div className="flex items-center space-x-2 ">
                  <LiaTimesSolid
                    className="h-5 w-5 cursor-pointer"
                    onClick={() => handleIconClick("category1")}
                  />
                  <DropdownChooseOption
                    name="Chọn"
                    optionNames={chooseOptions1}
                ...
                <div className="flex flex-wrap max-w-sm ml-5">
                  {selectedChooseOptions1.map((option, index) => (
                    <div
                      key={index}
                      className="flex items-center space-x-2 shadow-sm bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 p-2.5 mb-2 mx-2"
                    >
                      <p className="max-w-xl">{option}</p>
                      <LiaTimesSolid
                        className="h-5 w-5 cursor-pointer"
                        onClick={() =>
                          handleRemoveOption(option, setSelectedChooseOptions1)
                        }
                      />
                    </div>
                  ))}
          ...
          <div className="flex mb-4 space-x-10">
            ...
            {isLoading2 ? (
              ...
              <div
                id="choose-option2"
                className={`space-y-5 mt-7 ${isVisibleOption2 ? "" : "hidden"}`}
              >
                <div className="flex items-center space-x-2 ">
                  <LiaTimesSolid
                    className="h-5 w-5 cursor-pointer"
                    onClick={() => handleIconClick("category2")}
                  />
                  <DropdownChooseOption
                    name="Chọn"
                    optionNames={chooseOptions2}
                  ...
                  {selectedChooseOptions2.map((option, index) => (
                    ...
                    >
                      ...
                      <LiaTimesSolid
                        className="h-5 w-5 cursor-pointer"
                        onClick={() =>
                          handleRemoveOption(option, setSelectedChooseOptions2)
                        }
                      />
                    </div>
                  ))}
